...
/**
 * Check if a string is a valid Flashcat site.
 *
 * If the environment variable `DD_CI_BYPASS_SITE_VALIDATION` is
 * set, then it will return `true`.
 *
 * @param site an optional string.
 * @returns a boolean indicating if the provided site is valid a Flashcat site.
 */
export const isValidflashcatSite = (site?: string): boolean => {
  if (site === undefined) {
    return false
  }
