...
export const getRequestBuilder = (options: RequestOptions) => {
  ...
  return (args: AxiosRequestConfig) =>
    axios.create(baseConfiguration)(overrideArgs(args));
};
...
export const recursivelyRemoveUndefinedValues = <
  T extends Record<string, unknown>
>(
  object: T
): Partial<T> => {
  ...
  for (const [key, value] of Object.entries(object) as [
    ...
    if (typeof value === "object" && !Array.isArray(value)) {
      newObject[key] = recursivelyRemoveUndefinedValues(
        value as any
      ) as T[keyof T];
...
export const performSubCommand = (
...
export const filterSensitiveInfoFromRepository = (
...
export const filterAndFormatGithubRemote = (
  rawRemote: string | undefined
): string | undefined => {
  rawRemote = filterSensitiveInfoFromRepository(rawRemote);
  if (!rawRemote) {
    return rawRemote;
  }
  rawRemote = rawRemote.replace(
...
export const timedExecAsync = async <I, O>(
  f: (input: I) => Promise<O>,
  input: I
): Promise<number> => {
...
export const formatBytes = (bytes: number, decimals = 2) => {
  ...
  const formattedBytes = parseFloat(
    (bytes / Math.pow(bytesPerKB, i)).toFixed(numDecimals)
  );
...
export const parsePathsList = (paths: string | undefined): string[] => {
  ...
  return paths
    .split(",")
    .flatMap((path) =>
      glob.hasMagic(path) ? glob.sync(path, { dotRelative: true }) : [path]
    )
