...
Upload JavaScript sourcemaps to Flashcat to un-minify your errors.
...
This command will upload all JavaScript sourcemaps and their corresponding JavaScript bundles to Flashcat in order to un-minify front-end stack traces received by Flashcat.
...
* `--service` (required) should be set as the name of the service you're uploading sourcemaps for, and Flashcat will use this service name to find the corresponding sourcemaps based on the `service` tag set on the RUM SDK.
...
* `--disable-git` (default: false): prevents the command from invoking git in the current working directory and sending repository related data to Flashcat (hash, remote URL and the paths within the repository of the sources referenced in the sourcemap).
...
Example: With a remote `git@github.com:Flashcat/example.git`, links pointing to `https://github.com/Flashcat/example` are generated.
This behavior can be overridden with links to `https://gitlab.com/Flashcat/example` with the flag `--repository-url=https://gitlab.com/Flashcat/example`.
