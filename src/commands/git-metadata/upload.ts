...
import {apiHost, flashcatSite, getBaseIntakeUrl} from './api'
...
export class UploadCommand extends Command {
  ...
  public static usage = Command.Usage({
    ...
    description: 'Report the current commit details to FLASHCAT.',
    details: `
      This command will upload the commit details to FLASHCAT in order to create links to your repositories inside FLASHCAT's UI.\n
      See README for details.
    `,
    examples: [['Upload the current commit details', 'FLASHCAT-ci report-commits upload']],
  })
  ...
  private config = {
    apiKey: process.env.FLASHCAT_API_KEY ?? process.env.DD_API_KEY,
  }
  ...
  public async execute() {
    ...
    if (!this.config.apiKey) {
      this.logger.error(
        renderConfigurationError(
          new InvalidConfigurationError(`Missing ${chalk.bold('FLASHCAT_API_KEY')} in your environment`)
        )
      )
  ...
  private getSrcmapRequestBuilder(apiKey: string): RequestBuilder {
    return getRequestBuilder({
      ...
      headers: new Map([
        ['DD-EVP-ORIGIN', 'FLASHCAT-ci_git-metadata'],
