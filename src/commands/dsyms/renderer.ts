...
export const renderConfigurationError = (error: Error) =>
  chalk.red(`${ICONS.FAILED} Configuration error: ${error}.\n`);

export const renderInvalidDsymWarning = (dSYMPath: string) =>
  chalk.yellow(
    `${ICONS.WARNING} Invalid dSYM file, will be skipped: ${dSYMPath}\n`
  );

export const renderDSYMSlimmingFailure = (dSYM: Dsym, slice: ArchSlice) =>
  chalk.yellow(
    `${ICONS.WARNING} Failed to export '${slice.arch}' arch (${slice.uuid}) from ${dSYM.bundlePath}\n`
  );

export const renderFailedUpload = (
  dSYM: CompressedDsym,
  errorMessage: string
) => {
  ...
  return chalk.red(
    `${ICONS.FAILED} Failed upload dSYM for ${dSYMPathBold}: ${errorMessage}\n`
  );
};

export const renderRetriedUpload = (
  dSYM: CompressedDsym,
  errorMessage: string,
  attempt: number
) => {
  ...
  return chalk.yellow(
    `[attempt ${attempt}] Retrying dSYM upload ${dSYMPathBold}: ${errorMessage}\n`
  );
};
...
export const renderSuccessfulCommand = (
  ...
  if (results.get(UploadStatus.Failure)) {
    output.push(
      chalk.red(`${ICONS.FAILED} Some dSYMS have not been uploaded correctly.`)
    );
  } else if (results.get(UploadStatus.Skipped)) {
    output.push(
      chalk.yellow(`${ICONS.WARNING}  Some dSYMs have been skipped.`)
    );
    ...
    output.push(
      chalk.yellow(
        `${ICONS.WARNING} No dSYMs detected. Did you specify the correct directory?`
      )
    );
  }
  ...
  if (results.get(UploadStatus.Failure) || results.get(UploadStatus.Skipped)) {
    output.push(
      `Details about the found ${pluralize(statuses.length, "dSYM", "dSYMs")}:`
    );
    ...
    if (results.get(UploadStatus.Skipped)) {
      output.push(
        chalk.yellow(
          `  * ${pluralize(
            results.get(UploadStatus.Skipped)!,
            "dSYM was",
            "dSYMs were"
          )} skipped`
        )
    ...
    if (results.get(UploadStatus.Failure)) {
      output.push(
        chalk.red(
          `  * ${pluralize(
            results.get(UploadStatus.Failure)!,
            "dSYM",
            "dSYMs"
          )} failed to upload`
        )
...
export const renderCommandInfo = (
  ...
  if (dryRun) {
    fullStr += chalk.yellow(
      `${ICONS.WARNING} DRY-RUN MODE ENABLED. WILL NOT UPLOAD DSYMS\n`
    );
  }
  const startStr = chalk.green(
    `Starting upload with concurrency ${poolLimit}. \n`
  );
  fullStr += startStr;
...
export const renderUpload = (dSYM: CompressedDsym): string => {
  ...
  const objectName = dSYM.dsym.slices.map((slice) =>
    path.basename(slice.objectPath)
  )[0];
