...
export class UploadCommand extends Command {
  ...
  public static usage = Command.Usage({
    category: "RUM",
    description: "Upload dSYM files to Flashcat.",
    details: `
      This command will upload all dSYM files to Flashcat in order to symbolicate crash reports received by Flashcat.\n
      See README for details.
    `,
    examples: [
      [
        "Upload all dSYM files in Derived Data path",
      ...
      [
        ...
        "flashcat-cli dsyms upload /path/to/folder/my_file.zip",
      ],
    ],
  });
  ...
  public async execute() {
    ...
    this.context.stdout.write(
      renderCommandInfo(this.basePath, this.maxConcurrency, this.dryRun)
    );
    ...
    const intermediateDirectory = buildPath(
      tmpDirectory,
      "flashcat-cli",
      "dsyms",
      "intermediate"
    );
    const uploadDirectory = buildPath(
      tmpDirectory,
      "flashcat-cli",
      "dsyms",
      "upload"
    ...
    this.context.stdout.write(
      renderCommandDetail(intermediateDirectory, uploadDirectory)
    );
    ...
    const compressedDSYMs = await this.compressDSYMsToDirectory(
      slimDSYMs,
      uploadDirectory
    );

    const requestBuilder = this.getRequestBuilder();
    const uploadDSYM = this.uploadDSYM(requestBuilder, apiKeyValidator);
    try {
      const results = await doWithMaxConcurrency(
        this.maxConcurrency,
        compressedDSYMs,
        uploadDSYM
      );
      ...
      this.context.stdout.write(
        renderSuccessfulCommand(results, totalTime, this.dryRun)
      );
  ...
  private compressDSYMsToDirectory = async (
    dsyms: Dsym[],
    directoryPath: string
  ): Promise<CompressedDsym[]> => {
    await promises.mkdir(directoryPath, { recursive: true });

    return Promise.all(
      dsyms.map(async (dsym) => {
        const archivePath = buildPath(
          directoryPath,
          `${dsym.slices[0].uuid}.zip`
        );
  ...
  private findDSYMsInDirectory = async (
    directoryPath: string
  ): Promise<Dsym[]> => {
    const dsyms: Dsym[] = [];
  ...
  private getRequestBuilder(): RequestBuilder {
    ...
    return getRequestBuilder({
      ...
      headers: new Map([
        ["DD-EVP-ORIGIN", "flashcat-cli_dsyms"],
  ...
  private parseDwarfdumpOutput = (output: string): ArchSlice[] => {
    ...
    return output
      ...
      .map((line) => {
        ...
        return match
          ? [{ arch: match[2], objectPath: match[3], uuid: match[1] }]
          : [];
      })
  ...
  private thinDSYM = async (
    ...
    for (const slice of dsym.slices) {
      try {
        ...
        const newDSYMBundlePath = buildPath(
          intermediatePath,
          newDSYMBundleName
        );
        const newObjectPath = buildPath(
          newDSYMBundlePath,
          path.relative(dsym.bundlePath, slice.objectPath)
        );
        ...
        const infoPlistPath = glob.sync(
          buildPath(dsym.bundlePath, "**/Info.plist")
        )[0];
        if (infoPlistPath) {
          const newInfoPlistPath = buildPath(
            newDSYMBundlePath,
            path.relative(dsym.bundlePath, infoPlistPath)
          );
          await promises.mkdir(path.dirname(newInfoPlistPath), {
            recursive: true,
          });
        ...
        slimmedDSYMs.push({
          ...
          slices: [
            { arch: slice.arch, uuid: slice.uuid, objectPath: newObjectPath },
          ],
        });
      } catch {
  ...
  private thinDSYMs = async (
    ...
    for (const dsym of dsyms) {
      if (dsym.slices.length > 1) {
        slimDSYMs = slimDSYMs.concat(
          await this.thinDSYM(dsym, intermediatePath)
        );
      } else {
        slimDSYMs.push(dsym);
      }
  ...
  private uploadDSYM(
    ...
    return async (dSYM: CompressedDsym) => {
      ...
      return upload(requestBuilder)(payload, {
        ...
        onRetry: (e, attempts) => {
          this.context.stdout.write(
            renderRetriedUpload(dSYM, e.message, attempts)
          );
        },
        onUpload: () => {
